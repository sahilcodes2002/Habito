...
//   const [currentWeek, setCurrentWeek] = useState(startWeek(new Date(), { weekStartsOn: 1 }));
...
//     const task = prompt(`Add a task for ${day}, ${format(date, "MMMM d, yyyy")}:`);
...
//   // Function to handle date selection from the calendar (sets the week starting from Monday)
...
//       <h1 className="text-2xl font-bold mb-4 text-center">Weekly Task Planner</h1>

//       {/* Week Task List */}
//       <div className="overflow-y-auto h-72 week-calendar grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4">
//         {daysOfWeek.map((day, index) => {
//           const date = addDays(currentWeek, index); // Get the date for each day
//           const formattedDate = getFormattedDate(date);

//           return (
//             <div key={day} className=" day-column bg-white border p-4 rounded-lg shadow-lg">
//               <h2 className="font-bold text-center mb-4">
//                 {day[0]+day[1]+day[2]} - {format(date, "MMMM d").split(' ')[0][0]}{format(date, "MMMM d").split(' ')[0][1]}{format(date, "MMMM d").split(' ')[0][2]}/{format(date, "MMMM d").split(' ')[1]}
//               </h2>
...
//                     return<li key={idx} className={`text-sm overflow-x-auto mb-2 ${lightColors[randomNumber]} text-gray-700 border border-black px-1 rounded-md`}>{task}</li>}
//                 )
//                 ) : (
