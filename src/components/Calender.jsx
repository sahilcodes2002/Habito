...
const MyCalendar = ({addevent,setaddevent,calenderevents,setcalenderevents}) => {
  ...
  const handleAddEvent = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://honoprisma.codessahil.workers.dev/addevent", // Replace with your actual backend URL
        {
          title,
          color,
  ...
  useEffect(() => {
    async function getdata() {
      ...
      try {
        ...
        // }
        if(calenderevents){
          setEvents(formatEvents(calenderevents));
          return;
        }
        const response = await axios.get(`https://honoprisma.codessahil.workers.dev/getevents`, {
          headers: {
  ...
  const handleCompleteTask = async (task) => {
    ...
    try {
      // Make a POST request to add the event
      const response = await axios.post(
        "https://honoprisma.codessahil.workers.dev/deleteevents", // Replace with your actual backend URL
        {
          event_id: task.id,
        },
        {
  ...
  const handleDateChange = (field, value) => {
    if (field === "startDate") {
      setNewEvent((prevEvent) => ({
        ...prevEvent,
        startDate: value,
        startTime: prevEvent.startTime || "22:00", // Default to 10 PM if startTime is not set
        start: new Date(`${value}T${prevEvent.startTime || "22:00"}`),
      }));
      ...
      setNewEvent((prevEvent) => ({
        ...prevEvent,
        endDate: value,
        endTime: prevEvent.endTime || "22:00", // Default to 10 PM if endTime is not set
        end: new Date(`${value}T${prevEvent.endTime || "22:00"}`),
      }));
  ...
  return (
    <div className="z-0 md:px-5  rounded-md">
      <h1 className="text-2xl mt-4 font-bold text-center">{addevent?"Create Event":"Event Planner"}</h1>
      <div>
        <div className=" ">
          {/* Event form */}
          ...
          <div className="flex flex-col">
            ...
            <div className={`${addevent ? "" : "hidden"}`}>
              <div className="bg-slate-400 rounded p-2 text-center">
                ...
                <div className="w-full flex justify-center">
                  <button
                    className={`mt-4 bg-[rgb(47,141,113)] hover:bg-[rgb(18,107,70)] text-white py-[10px] px-20 md:py-[13px] md:px-20 rounded-md transition duration-300 font-semibold border-none text-sm`}
          ...
          <div
            style={{ height: 250 }}
            className={` flex justify-center ${addevent ? "hidden " : "flex justify-center"}`}
          >
            <Calendar
              localizer={localizer}
              events={events}
              ...
              //toolbar={false} // Remove Month, Week, Date options
              components={{
