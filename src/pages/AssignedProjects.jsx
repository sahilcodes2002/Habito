  ...
  useEffect(() => {
    ...
    async function reget() {
      try {
        ...
        axios
          .get(
            `https://honoprisma.codessahil.workers.dev/getassignedprojects`,
            {
              headers: {
...
function Usercodes({ isSidebarOpen, isLoading, projectinfo, setprojectinfo }) {
  ...
  const handleupdate = async (id, important) => {
    ...
    try {
      const res = await axios.post(
        `https://honoprisma.codessahil.workers.dev/updateassignedprojectimportant`,
        {
          id: id,
          important,
        },
      ...
      setProjects((prevProjects) =>
        prevProjects.map((project) => {
          if (project.projectinvite[0].id === id) {
            // Create a new project object with the updated 'important' value
            return {
              ...project,
              projectinvite: [
                {
                  ...project.projectinvite[0],
                  important: res.data.res.important, // Update important
                },
              ],
            };
          } else {
                    ...
                    <div>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          const important =
                            project.projectinvite[0].important || false;
                          handleupdate(project.projectinvite[0].id, !important);
                        }}
                      ...
                      >
                        <svg
                          ...
                          xmlns="http://www.w3.org/2000/svg"
                          fill={`${
                            project.projectinvite[0].important
                              ? "goldenrod"
                              : "white"
                          }`}
                          viewBox="0 0 24 24"
...
const HorizontalSlider = ({ isSidebarOpen, isLoading, projectinfo }) => {
  ...
  useEffect(() => {
    if (projectinfo) {
      ...
      for (const a of x) {
        if (a.projectinvite[0].important) {
    ...
    <div className="z-0 bg-blue-200 rounded-lg ml-10">
      <div className="px-5 md:px-7 ml-1 py-2 flex justify-center h-full space-x-5 bg-white rounded-md">
        ...
        <div className="flex flex-col justify-center px-2">
          <div className="text-sm text-gray-600 ">important tasks/projects</div>
