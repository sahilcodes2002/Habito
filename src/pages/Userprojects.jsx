...
function Usercodes({ isSidebarOpen, isLoading, projectinfo, setprojectinfo }) {
  ...
  const [delitingid, setdelitingid] = useState(-1);
              ...
              {filteredGroupedProjects[date] &&
                filteredGroupedProjects[date].map((project) => (
                  ...
                  >
                    ...
                    {delitingid == project.id &&<div>
                      <h3 className="font-semibold text-gray-600">
                        {"Delete "}{`"`}<span className="text-green-800">{project.title}</span>{`": `}
                        <span><button onClick={(e)=>{
                          e.stopPropagation();
                          handleDelete(project.id);
                        }} className="mr-7 ml-4 underline text-blue-800">Yes</button><button onClick={(e)=>{
                          e.stopPropagation();
                          setdelitingid(-1);
                        }} className=" underline text-red-800">No</button></span>
                      </h3>
                      {/* <p className="text-sm text-gray-500">
                        {project.description}
                      </p> */}
                    ...
                    <div>
                      {delitingid != project.id &&<button
                        onClick={(e) => {
                          e.stopPropagation();
                          const important = project.important||false;
                          handleupdate(project.id, !important);
                        }}
